...
## Introduction
The Python programme is a simple Command Line Interface (CLI) inventory management system. It can recieve input of product details and write these into a JSON file for long-term storage. Within the JSON file, each product is stored as a dictionary. The keys are the data types (i.e. Product ID, Product Name, Product Quantity, and Price). The values are the entries inputted by the user, along with the automatically generated product ID. The different dictionaries are stored as a list in the JSON file.

The product names cannot be changed, but product quantities and prices can be updated.

### Use Cases
The inventory management system will be useful for storing the price and quantity details of small to medium amounts of inventory. While there is no upper limit for the number of inventory items, to retain its speed, the programme can optimally manage a few hundred entries at most.
The pogramme will be helpful for a small business warehouse or a private household's kitchen pantry.
...
python C:\path\to\python Inventory_Management_System_Consolidation_Task.py
...
(Acceptable product name inputs include: jam, cheese, chocolate, apricot jam, cheddar cheese, dark chocolate, milk-UHT, ChipsAhoy! and 7UP).
If the first character in the name inputted is a letter, Python automatically capitalises it before writing it into the JSON file.
...
In the present version, products cannot be deleted from the list through the programme. 
